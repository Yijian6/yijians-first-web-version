...
就`quit`而言，我们要
	...
	②处理这个内置命令
❹写代码
	①识别：直接用`strcmp(string1，string2)`(string compare)函数就可以,内部return 1;
	②执行：quit命令，直接用exit(0)退出就可以。
❺示例代码如下：
```C
...
我们先看看`waitfg()`，这个函数让我们完成啥吧？
注释告诉我们：
![[Pasted image 20260802163851.png]]
🌟只要一个进程还是前台进程，我们就要一直等待它结束。有点复杂啊？
❶我们要判断一个进程是不是前台进程。
❷一个进程的结束，对于Shell而言，是 `fork()`完了之后，子进程的状态的变化，操作系统，会发生SIGCHLD,这还设计到了另一个函数`sigchld_handler()`.
有点复杂啊？
不会处理。
不过，
毕竟trace2只让我们处理一个内置命令。接受quit,解析、执行quit、退出。(我们的`parseline()`和`builtin_cmd()`已经实现)现在我们已经就把trace02通过了，那我们先把`waitpid()`和`sigchld_handler()`空着吧，之后再写。
所以父进程的框架就成型了：
```C
//父进程处理
//前台进程
...
```
完成拼起来，填入`eval()`就是：
![[Pasted image 20260803125915.png]]

来吧，make、make test02和make rtest02，比较结果。发现也是直接过了！输出一模一样，都在最后输出完之后，自动退出了。
